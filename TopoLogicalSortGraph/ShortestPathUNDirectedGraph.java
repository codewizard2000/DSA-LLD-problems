...
public class ShortestPathUNDirectedGraph {
    public int[] shortestPath(int[][] edges,int n,int m ,int src) {
        ...
        for(int i = 0;i<m;i++) {
            adj.get(edges[i][0]).add(edges[i][1]);//connecting the edge for each node 
        ...
        while(!q.isEmpty()) {
            int node = q.peek();
            q.remove();
            for(int it : adj.get(node)) {// check if the parent node distance +current distance <  previous distance
